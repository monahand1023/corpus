...
Out-of-the-box formats: **markdown**, **text**, **pdf** (`[pdf]` extra), **html** (`[html]` extra), **docx** (`[docx]` extra), **xlsx** (`[xlsx]` extra), **rtf** (`[rtf]` extra), **pptx** (`[pptx]` extra), **csv** / **tsv** (no extra needed), **zip** (archives of any of the above), **aup3** (Audacity 3 projects — layout indexed, speech via `corpus-transcribe`) — see [Built-in connectors](#built-in-connectors). For anything else (Slack exports, JSON dumps, EPUB…), write a small connector: [`docs/adding_a_source.md`](docs/adding_a_source.md).
...
closes it: point it at a folder of recordings (Audacity `.aup3` projects
included) and it writes a sidecar database that is then a source like any other.
It is hours of local compute, so it has its own dry run and confirmation rather
than running because a folder happened to contain an `.mp4`.

```bash
corpus-transcribe ~/Videos --dry-run          # how much audio, how long, what's left
corpus-transcribe ~/Videos --min-seconds 15   # skip Live Photo clips (81% of one library)
corpus-transcribe ~/Videos                    # do it (safe to interrupt and re-run)
```
...
Speech models invent text on silence, confidently, so every transcript is
filtered on its TEXT (caption boilerplate, looping, impossible speech rate,
optionally unexpected language) rather than on the model's scores. Needs
`ffmpeg` plus the `[transcribe]` extra, and `[transcribe-mlx]` for the shipped
Apple-Silicon model.

- [docs/transcription.md](docs/transcription.md): the dry run, `--min-seconds`,
  re-filtering after a threshold change (`--refilter`, `--redo-stale`),
  interrupting, backends, Audacity projects.
- [docs/transcript_quality.md](docs/transcript_quality.md): why the filters work
  the way they do, with the measurements behind each.
...
| `aup3` | `**/*.aup3` | — (stdlib `sqlite3`; numpy + ffmpeg to transcribe or extract) | Audacity 3 project files. One document per project with its real duration, sample rate and tracks, read from the project's own layout. To search what is *said* in them, run `corpus-transcribe` over the folder: `.aup3` is transcribed like any recording (tracks mixed to mono) and indexed via a `transcripts` source. `corpus.connectors.aup3.extract_audio()` writes the same mix as a playable WAV/FLAC/MP3. The source `.aup3` is opened read-only and never modified. `sample_rate` / `channels` are used only for a project whose layout can't be read. |
